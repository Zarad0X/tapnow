...
import { arrangeNodesByGraphLayers } from './canvas/layout.js';
import {
        ...
        function TapnowApp() {
            ...
            const pollImageTask = (taskId, taskIdForPoll, baseUrl, apiKey, w, h, sourceNodeId, attempt = 0, isBananaModel = false) => {
                ...
                .then((text) => {
                    ...
                    let { images, source: imageSource } = extractAsyncImageItems(data);
                    if (images.length > 0 && imageSource) {
                        console.log(`[Async Image] 从 ${imageSource} 提取到图片:`, images.length, '张');
                    }
                            ...
                            if (hItem.id === taskId) {
                                ...
                                if (asyncImageStatus === ASYNC_IMAGE_STATUS.COMPLETED) {
                                    ...
                                    if (images && images.length > 0) {
                                        // 提取图片URL，支持多种字段名和格式
                                        const imageUrls = normalizeImageItemsToUrls(images);
...
- **口播提取**要依靠视频中的音频内容，如果视频没有声音则留空。`;
            ...
            const autoArrangeNodes = () => {
                ...
                const updatedNodesMap = arrangeNodesByGraphLayers({
                    nodesToArrange,
                    connections: connectionsRef.current,
                });
                    ...
                    <div className="absolute inset-0 pointer-events-none overflow-visible w-full h-full">
                        <svg className="absolute inset-0 overflow-visible w-full h-full">
                            {visibleConnections.map((conn) => {
                                ...
                                const endY = getNodeInputAnchorY({
                                    node: toNode,
                                    inputType: conn.inputType,
                                    connectionId: conn.id,
                                    relevantConnections: relevantConns,
                            ...
                            const startY = getNodeInputAnchorY({
                                node,
                                inputType: connectingInputType,
                                relevantConnections: connectionsByNode.to.get(node.id) || [],
                                modelConfig: apiConfigsMap.get(node.settings?.model),
                            });
